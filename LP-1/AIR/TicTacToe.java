...
{ 
  ...
  static String humanPlayer; //Human holds any symbol X or O 
  static String aiPlayer; 
  static int moveI,moveJ; //board indexes of current AI moves - used in printBoard() 
 
  public static void init() //Code to Initialize board 
   { 
	  for (int i=0;i<3;i++) 
		  for(int j=0;j<3;j++) 
		  { 
 ...
 public static void selectSymbol() //Take input Symbol X or O from Human Player 
 {
	 String input = ""; 
	 System.out.println("Choose any value from X or O : "); 
	 input = in.next(); 
	 while(!input.equalsIgnoreCase("x") && !input.equalsIgnoreCase("o")) 
	 { 
		 System.out.println("\nInvalid Input!!! (Select Only X or O)\n"); 
		 System.out.println("Choose any value from X or O : "); 
		 input = in.next(); 
	 } 
	 ...
	 { 
		 ...
		 { 
			 System.out.println("AI Player marked "+aiPlayer+" at position "+(moveI+1)+"-"+(moveJ+1)); 
					 System.out.println("Your Move (Use Numpad to select Position): "); 
		 } 
		 else 
		 { 
			 System.out.println("Your Move (Use Numpad to select Position): "); 
 ...
 public static boolean validMove(int boardNumber) //Check if position on board is empty or not 
 { 
	 ...
	 do{ 
		 ...
		 { 
			 ...
			 if((humanPlayerInput < 1 || humanPlayerInput > 9)) 
				 System.out.println("Invalid Input (Only Select Numpad 1-9)"); 
			 else if(!validMove(humanPlayerInput)) 
				 System.out.println("Invalid Move! Place is Already Marked!! Select other Place"); 
		 } 
		 catch(Exception e) 
		 { 
...
public static String checkWin(String newBoard[][]) 
	 { //Accepts a board and Checks if User/AI has won, or draw 
...
{ //keep playing while value returned from checkWin() is "nothing" 
	moveCount = 0; 
	printBoard(); 
